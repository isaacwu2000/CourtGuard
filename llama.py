...
class Court_Detector(weave.Model):
    ...
    async def predict(self, prompt):
        ...
        synthesis = await loop.run_in_executor(None, llama, get_prompt(prompt_name = "synthesis_prompt", user_input = prompt, benign = benign, adversarial = adversarial))
        verdict = await loop.run_in_executor(None, llama, get_prompt(prompt_name = "verdict_prompt", verdict_info = synthesis))
